...
def compute_psnr(folder_Gen, folder_GT, test_Y=True, crop_border=4):
    # Configurations
    ...
    print('image list:', len(img_list_gt))
    print('image list:', len(img_list_gen))
    ...
    for i, img_path in enumerate(img_list_gt):
        print("inside psnr.py, image no. %d"%i)
        ...
        im_Gen = cv2.imread(img_list_gen[i]) / 255.
        print(im_Gen.shape)
        ...
        # crop borders
        if im_GT_in.ndim == 3:
            h, w = im_GT_in.shape[:2]
            cropped_GT = im_GT_in[crop_border:h-crop_border, crop_border:w-crop_border, :]
            cropped_Gen = im_Gen_in[crop_border:h-crop_border, crop_border:w-crop_border, :]
...
if __name__ == '__main__':
    psnr = compute_psnr(sys.argv[1], sys.argv[2])
    print(psnr)
