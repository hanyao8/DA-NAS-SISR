...
def patch_based_infer(real_A,model,patch_shape):
    assert real_A.shape[0]==1
    assert real_A.shape[1]==3
    image_height = patch_shape[0]
    image_width = patch_shape[1]
    #assert image_height == 32 #temporary
    #assert image_width == 32 #temporary

    n_patches_vertical = int(real_A.shape[-2]/image_height)
    n_patches_horizontal = int(real_A.shape[-1]/image_width)
    fake_B = torch.zeros((1,3,4*real_A.shape[-2],4*real_A.shape[-1]))

    for patch_v_idx in range(n_patches_vertical):
        real_A_row = real_A[0,:,
            patch_v_idx*image_height:(patch_v_idx+1)*image_height,
            :n_patches_horizontal*image_width]
        real_A_row_batched = torch.zeros((n_patches_horizontal,3,image_height,image_width))
        for patch_h_idx in range(n_patches_horizontal):
            real_A_row_batched[patch_h_idx,:,:,:] = real_A_row[:,:,patch_h_idx*image_width:(patch_h_idx+1)*image_width]
        fake_B_row_batched = model(real_A_row_batched).data.float().clamp_(0, 1)
        for patch_h_idx in range(n_patches_horizontal):
            fake_B[0,:,
                patch_v_idx*4*image_height:(patch_v_idx+1)*4*image_height,
                patch_h_idx*4*image_width:(patch_h_idx+1)*4*image_width
                ] = fake_B_row_batched[patch_h_idx,:,:,:]

    vertical_remainder = real_A.shape[-2] - n_patches_vertical*image_height
    if vertical_remainder > 0:
        real_A_row = real_A[0,:,-image_height:,:n_patches_horizontal*image_width]
        real_A_row_batched = torch.zeros((n_patches_horizontal,3,image_height,image_width))
        for patch_h_idx in range(n_patches_horizontal):
            real_A_row_batched[patch_h_idx,:,:,:] = real_A_row[:,:,patch_h_idx*image_width:(patch_h_idx+1)*image_width]
        fake_B_row_batched = model(real_A_row_batched).data.float().clamp_(0, 1)
        for patch_h_idx in range(n_patches_horizontal):
            fake_B[0,:,-4*image_height:,
                patch_h_idx*4*image_width:(patch_h_idx+1)*4*image_width
                ] = fake_B_row_batched[patch_h_idx,:,:,:]

    horizontal_remainder = real_A.shape[-1] - n_patches_horizontal*image_width
    if horizontal_remainder > 0:
        real_A_col = real_A[0,:,:n_patches_vertical*image_height,-image_width:]
        real_A_col_batched = torch.zeros((n_patches_vertical,3,image_height,image_width))
        for patch_v_idx in range(n_patches_vertical):
            real_A_col_batched[patch_v_idx,:,:,:] = real_A_col[:,patch_v_idx*image_height:(patch_v_idx+1)*image_height,:]
        fake_B_col_batched = model(real_A_col_batched).data.float().clamp_(0, 1)
        for patch_v_idx in range(n_patches_vertical):
            fake_B[0,:,
                patch_v_idx*4*image_height:(patch_v_idx+1)*4*image_height,
                -4*image_width:
                ] = fake_B_col_batched[patch_v_idx,:,:,:]

    if vertical_remainder>0 and horizontal_remainder>0:
        real_A_corner = real_A[:,:,-image_height:,-image_width:]
        fake_B_corner = model(real_A_corner).data.float().clamp_(0, 1)
        fake_B[0,:,-4*image_height:,-4*image_width:] = fake_B_corner[0,:,:,:]
