...
class Architect(object):
    ...
    def step(self, input_train, target_train, input_valid, target_valid, arch_optimizer, curr_epoch, eta=None, network_optimizer=None, unrolled=False):
        print('architect.py Architect.step()')
        arch_optimizer.zero_grad()
        # arch_optimizer_gamma.zero_grad()
        if unrolled:
        ...
        if loss_flops != 0:
            loss += loss_flops
        print('loss:', loss)
        print('loss_flops:', loss_flops)

        loss.backward()
        #print('gamma group grad:', self.model.module.group_gamma.grad)
        print('curr_epoch:', curr_epoch)
        arch_optimizer.step()
        # arch_optimizer_gamma.step_adam_hapg(curr_epoch=curr_epoch, alpha=self.model.module.alpha_sparsity, Lambda=self.model.module.Lambda)
        # arch_optimizer.step_adam_hapg()
